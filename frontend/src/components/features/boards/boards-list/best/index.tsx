...
import useBoardsOfTheBest from "./hook";
import Image from "next/image";
import styles from "./styles.module.css";
import { formatInTimeZone } from "date-fns-tz";

export default function BoardsOfTheBest() {
  const { best, onClickBoards } = useBoardsOfTheBest();

  if (!best) {
    return (
      <div className={styles.BoardsOfBest}>
        {Array.from({ length: 4 }).map((_, index) => (
          <div className={styles.BoardCards} key={index}>
            <Image
            ...
            <div className={styles.Card}>
              <div className={styles.CardName}>
                <p className={styles.CardTitle}>로딩중 ... </p>
                <div>
                  <Image
                    src={`/images/profile/9.svg`}
                    alt={`loading_profile${index + 1}`}
                    width={0}
                    height={0}
                    sizes="100vw"
  ...
  return (
    <div className={styles.BoardsOfBest}>
      {best?.map((el: Board, index: number) => (
        <div className={styles.BoardCards} key={el._id} id={el._id} onClick={onClickBoards}>
          <Image
            src={
              el.images?.[0]
                ? `https://storage.googleapis.com/${el.images[0]}`
                : `/images/img-${index + 1}.svg`
            }
            alt={`BoardsOfTheBest_image${index + 1}`}
            width={0}
            height={0}
            sizes="100vw"
            className={styles.BestImage}
          ...
          <div className={styles.Card}>
            <div className={styles.CardName}>
              ...
              <div>
                <Image
                  src={`/images/profile/${index + 1}.svg`}
                  alt={`BoardsOfTheBest_profile${index + 1}`}
                  width={0}
                  height={0}
                  sizes="100vw"
                  className={styles.profile}
